...
## 最小の操作数
 ...
 - 配列の各要素に対して+1か-1を選択するような場合は, とりあえず全て-1にしておいて, +1にするものを貪欲に取ることができる場合がある.

## 数学
 - 約数列挙が複数必要 -> エラストテレスでO(nlogn)
 - LCM(k, n)はk x (n / gcd(k, n)) <= k x (n / (kとnの共通約数))
 ...
 - 和を固定して積を最大化する時はできるだけ2か3(特に3)を使うのが良い. (機能的に分かる)
 - 循環小数が循環する桁数: 同じ商ではなく余りが同じになるところ(project euler26)
 - 大きすぎる値の比較: logをとる
...
## 数列
 ...
 - 2項間漸化式は解ける
 - 値があまり大きくない数列について, a[i] + a[j] = kとなるi,jの組の個数が知りたい->畳み込み
 (aとaを畳み込んで, 2*a[i]の位置から1を引いて, 結果を2で割ると重複が除ける)

## クエリ
 - 先読みできる場合は座標圧縮して大きい方か小さい方か見れる
 - 2次元BITなどが使いたくなる場合は平面走査でできるかも
 - 配列を2つのチームに分けるbit全探索: 片方のチームの値でsortするともう片方は単調減少. -> A, Bそれぞれで2分探索してUNIONとるなどができる.
...
## グラフ
 ...
 - 長さkの単純パスがあるか? -> グラフをk色に塗る時全て違う色のパスができるか? -> 色が割り当てられているならbit DP (dp[i][j]: 使った色がiで, 今jにいる)
 頂点にk個の色をrandomに割り当てると, 1回の成功確率は小さいが何回も反復すると成功確率は高い.
 - マッチング: どの頂点を使ったかのbitDPでもできる.
...
## 木
 ...
 - 木上のsumクエリなどは, segment treeを使わなくてもimosみたいに累積和を葉から押し出せばできるかも.
 (sum[u]++, sum[v]++, sum[lca(u, v)]--, sum[par[lca(u, v)]]--)
 - 頂点に対して値を加算する場合は, 上のようにLCAとLCAの親に対して引く. 辺なら2 * LCAを引く
...
## ダブリング
 - 操作回数が多すぎてダブリングできそうにない時は, そんなに回数が必要か考える.
 例えば十分大きい回数だと偶奇しか関係なかったら...？
...
## その他
 ...
 - priority_queueは便利
 - 集合の最大値を取り出したいだけの時などはpriority_queueが速い
 - Ax+By<=Cとい条件を満たすy(整数)を求める時に, y<=(C-Ax)/Bだが, C-Ax<0でも(C-Ax)/B=0となることがあるので注意
 - HW<=Nの時少なくとも片方は<=sqrt(N)
 - 何かの存在などの探索したいが探索範囲が広い時: できるだけ範囲を絞れる絞り方をする.
 (ex. グラフが長さ5の頂点1を含む単純閉路を含むか -> 頂点1の隣接を探索するのではなく, 頂点1に隣接していない辺を探索する)
